...
const Heatmap = () => {
  ...
  function process(results) {
    ...
    minYear = new Date(
      results[results.length - 1].creationTimeSeconds * 1000
    ).getUTCFullYear();
  ...
  function getStats(data) {
    ...
    document.getElementById("stats").innerHTML =
      ...
      " has made " +
      stats["sum"] +
      " submissions over a period of " +
      stats["numdays"] +
      " different days.";
    document.getElementById("stats").innerHTML +=
      "<br>Average number of submissions are " + stats["avg"];
    document.getElementById("stats").innerHTML +=
      "<br>Max Submissions in a day are " + stats["maxsub"];
    return stats;
  }
    ...
    results.forEach(function(result) {
      if (result.verdict === "OK") {
        ...
        if (count[year] === undefined) {
          count[year] = Array.apply(null, Array(54)).map(
  ...
  function createLCStructure(data) {
    ...
    // Create axes
    var hAxis = d3.axisBottom(xScale).ticks(20);
    ...
    document
      .getElementById("line-year")
      .addEventListener("change", populateLineChartWithData, false);
    ...
    function populateLineChartWithData() {
      lineGraph
        ...
        .duration(1200)
        .ease("elastic")
        .attr("d", lineFunction(data[this.value]));
      circles
        .data(data[parseInt(this.value)])
        .transition()
        .ease("back")
        .duration(750)
        .delay(function(d, i) {
